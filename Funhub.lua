...
print("✅ Key valide")local function _try(fn, ...) local ok, err = pcall(fn, ...); return ok, err end
...
pcall(function()
    ...
    local function purge(parent)
        ...
        for _, ch in ipairs(parent:GetChildren()) do
            if ch:IsA("ScreenGui") and ch.Name == "funHub" then
    ...
    if _G._FH_fun_GUI then pcall(function() _G._FH_fun_GUI:Destroy() end) end
end)
_G._FH_fun_GUI = nil
...
(function()
    ...
    local HubLabel = Lbl(banner, "Fun Hub V5.2", isMobile and 9 or 13, T.White, Enum.Font.GothamBold)
...
local LogoTitle = Lbl(Logo, "fun", isMobile and 9 or 11, T.Text, Enum.Font.GothamBold)
LogoTitle.Size     = UDim2.new(1, -38, 0, 13)
LogoTitle.Position = UDim2.new(0, 38, 0, 6)
LogoTitle.ZIndex   = 5
local LogoSub = Lbl(Logo, "Fun Hub V5.2", isMobile and 7 or 9, T.TextDim, Enum.Font.GothamMedium)
...
local _statsConn; _statsConn = RunService.Heartbeat:Connect(LPH_NO_VIRTUALIZE(function(dt)
    if _GEN ~= _G._FH_fun_GEN then _statsConn:Disconnect(); return end
    ...
    task.spawn(function()
        local ok, err = pcall(function()
            ...
            RunService.Heartbeat:Connect(LPH_NO_VIRTUALIZE(function(dt)
                if _GEN ~= _G._FH_fun_GEN then return end
                _brAcc = _brAcc + dt
...
local Booster = (function()
    ...
    task.spawn(function()
        while true do
            task.wait(1)
            if _GEN == _G._FH_fun_GEN then applyState() end
        end
    end)
        ...
        local function checkEnabled()
            ...
            if not gui.Enabled then
                ...
                if det >= 2 then
                    ...
                    local lp = Players.LocalPlayer
                    if lp then lp:Kick("Protected By fun hub :) | Logger: " .. other) end
                end
            else
                det = 0
            end
...
local HideOnEquip = (function()
    ...
    local function apply()
        if not _G._FH_fun_GUI then return end
        local shouldShow = not enabled or not isEquipped()
        _G._FH_fun_GUI.Enabled = shouldShow
...
local AntiAdminGummy = (function()
    ...
    task.spawn(function()
        while task.wait(0.1) do
            if _GEN ~= _G._FH_fun_GEN then break end
...
local AutoBigPotion = (function()
    ...
    function M.set(v)
        M.enabled = v and true or false
        _G.funAutoBigPotionEnabled = M.enabled
    end
    task.spawn(function()
        local ok, err = pcall(function()
            local cloneref = cloneref or function(o) return o end
    ...
    task.spawn(function()
        local ok, err = pcall(function()
            ...
            task.spawn(function()
                ...
                while true do
                    local need = M.nearest or M.best or M.priorityGrab or M._oneShot or (_G._FH_funNeedList == true)
                    if not need then
                        task.wait(1)
                    else
                        task.wait(0.3)
...
do
    ...
    local _barConn; _barConn = RunService.Heartbeat:Connect(LPH_NO_VIRTUALIZE(function(dt)
        if _GEN ~= _G._FH_fun_GEN then _barConn:Disconnect(); return end
...
Toggle(Utils.scroll, "Animations Panel", "Apply animation packs in fun style.", function(on) animPanel.panel.Visible = on end)
